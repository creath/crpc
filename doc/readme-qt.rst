CereiPayCoin-qt: Qt4 GUI for CereiPayCoin
===============================
...
Alternatively, install Qt Creator and open the `CereiPayCoin-qt.pro` file.

An executable named `CereiPayCoin-qt` will be built.
...
A warning for people using the *static binary* version of CereiPayCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of CereiPayCoin is linked against libdb5.3.
...
installed causes CereiPayCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
