...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("CereiPayCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CereiPayCoinTEST2\r");
            Send(hSocket, "WHO #CereiPayCoinTEST2\r");
        } else {
            // randomly join #CereiPayCoin00-#CereiPayCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #CereiPayCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #CereiPayCoin%02d\r", channel_number).c_str());
        }
